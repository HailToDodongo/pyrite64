...
function processFontRowMajor(png) {
    // Output: flat array, row-major, scanline-interleaved across all chars
    const out = [];
        ...
        for (let y = 0; y < CHAR_HEIGHT; ++y) {
            for (let charCol = 0; charCol < CHARS_PER_ROW; ++charCol) {
                for (let x = 0; x < CHAR_WIDTH; x += 2) {
                    const px1x = charCol * CHAR_WIDTH + x;
...
function writeHeaderRowMajor(fontData) {
    let out =
...
namespace FONT8x8
{
    ...
    // Data is row-major, scanline-interleaved across all chars
...
fs.createReadStream(INPUT_PNG)
    ...
    .on('parsed', function() {
        ...
        const fontData = processFontRowMajor(this);
        writeHeaderRowMajor(fontData);
    });
